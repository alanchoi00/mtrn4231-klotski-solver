...
if window_exists "$TITLE1"; then
  ...
  gnome-terminal --title="$TITLE1" -- bash -lc "ros2 launch ur_robot_driver ur_control.launch.py ur_type:=ur5e robot_ip:=yyy.yyy.yyy.yyy initial_joint_controller:=scaled_joint_trajectory_controller use_fake_hardware:=true launch_rviz:=false; exec bash" &
fi
...
if window_exists "$TITLE3"; then
  ...
  gnome-terminal --title="$TITLE3" -- bash -lc "ros2 run tf2_ros static_transform_publisher 1.30938 0.0206053 0.670571 -0.398486 0.00254305 0.917119 0.00974536 base_link camera_link; exec bash" &
fi
