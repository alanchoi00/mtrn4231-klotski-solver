...
def calculate_marker_homography(
    ...
    board_top_left_px = inner_corner(board_tl_marker_id)
    board_top_right_px = inner_corner(board_tr_marker_id)
    board_bottom_left_px = inner_corner(board_bl_marker_id)
    board_bottom_right_px = inner_corner(board_br_marker_id)
    ...
    src_pts = np.array(
        [
            board_top_left_px,
            board_top_right_px,
            board_bottom_right_px,
            board_bottom_left_px,
        ],
        dtype=np.float32,
    )
...
def warp_image_to_board(
    ...
    h_mat, size = calculate_marker_homography(
        aruco_infos,
        board_width_cells,
        board_height_cells,
        board_tl_marker_id,
