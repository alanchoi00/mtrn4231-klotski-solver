...
class HSVTestSense(Node):
    def __init__(self):
        ...
        self.image_sub = self.create_subscription(
            Image, "/camera/camera/image_raw", self.image_callback, 10
        )
        ...
        self.get_logger().info(
            f"HSV Test Sense Node started. Results will be saved to: {self.csv_filename}"
        )
    ...
    def calculate_mask_statistics(self, masks, image_shape):
        ...
        for color, mask in masks.items():
            ...
            stats[color] = {"area": area, "percentage": percentage}
        ...
        while rclpy.ok():
            ...
            if key == ord("q") or key == 27:  # 'q' or ESC
                ...
                if current_image is not None:
                    ...
                    test_id = self.save_to_csv(
                        masks, lighting_condition=lighting, notes=notes
                    )
                    ...
                    if masks is not None:
                        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                        cv2.imwrite(
                            f"{self.output_dir}/original_{test_id}_{timestamp}.png",
                            current_image,
                        )

                        for color, mask in masks.items():
                            cv2.imwrite(
                                f"{self.output_dir}/mask_{color}_{test_id}_{timestamp}.png",
                                mask,
                            )
                ...
                if current_image is not None:
                    masks = self.create_color_masks(current_image)
                    board_success, service_success = (
                        self.capture_board_with_current_hsv()
                    )

                    if service_success:
                        lighting = input(
                            "Enter lighting condition for board capture (optional): "
                        ).strip()
                        notes = input(
                            "Enter notes for board capture (optional): "
                        ).strip()
                        self.save_to_csv(
                            masks,
