...
//     : Node("arm_manipulator"), board_position_received_(false) {
//   declareAndLoadParameters();
...
//   // Create subscriber for board position from camera
//   if (use_dynamic_board_position_) {
//     board_position_sub_ =
//         this->create_subscription<geometry_msgs::msg::PointStamped>(
//             "/vision/board_center",  // Topic name - adjust as needed
//             10, std::bind(&ArmManipulator::boardPositionCallback, this, _1));

//     RCLCPP_INFO(
//         this->get_logger(),
//         "Subscribed to /vision/board_center for dynamic board position");
//     RCLCPP_WARN(
//         this->get_logger(),
//         "Waiting for board position from camera before accepting goals...");
//   } else {
//     RCLCPP_INFO(this->get_logger(),
//                 "Using static board position from config file");
//     board_position_received_ = true;  // Allow immediate operation
//   }
...
//               planner_id_.c_str());
//   RCLCPP_INFO(this->get_logger(),
//               "Height parameters - Board: %.3fm, Block: %.3fm, Gripper:
//               %.3fm", board_height_, block_height_, gripper_height_);
//   RCLCPP_INFO(
//       this->get_logger(),
//       "Calculated heights - Approach: %.3fm, Grip: %.3fm, Retreat: %.3fm",
//       approach_height_, grip_height_, retreat_height_);
//   RCLCPP_INFO(this->get_logger(), "Initial board center position: (%.3f,
//   %.3f)",
//               board_center_x_, board_center_y_);
...
//   auto col_object_table = generateCollisionObject(2.4, 1.2, 0.04, 0.8, 0.3,
//                                                   -0.04, frame_id, "table");
//   auto col_object_ceiling = generateCollisionObject(2.4, 2.4, 0.04, 0.85,
//   0.25,
//                                                     1.5, frame_id,
//                                                     "ceiling");
...
// void ArmManipulator::boardPositionCallback(
//     const geometry_msgs::msg::PointStamped::SharedPtr msg) {
//   RCLCPP_INFO(this->get_logger(),
//               "Received board position in frame '%s': (%.3f, %.3f, %.3f)",
//               msg->header.frame_id.c_str(), msg->point.x, msg->point.y,
//               msg->point.z);

//   // Transform the point from camera frame to base_link frame
//   geometry_msgs::msg::PointStamped transformed_point =
//       transformPoint(*msg, "base_link");

//   if (transformed_point.header.frame_id.empty()) {
//     RCLCPP_ERROR(this->get_logger(),
//                  "Failed to transform board position to base_link");
//     return;
//   }

//   // Update board center position
//   board_center_x_ = transformed_point.point.x;
//   board_center_y_ = transformed_point.point.y;

//   if (!board_position_received_) {
//     RCLCPP_INFO(this->get_logger(),
//                 "✓ Board position received! System ready to accept goals.");
//   }

//   board_position_received_ = true;

//   RCLCPP_INFO(this->get_logger(),
//               "Updated board center in base_link: (%.3f, %.3f)",
//               board_center_x_, board_center_y_);
// }

// geometry_msgs::msg::PointStamped ArmManipulator::transformPoint(
//     const geometry_msgs::msg::PointStamped& point_in,
//     const std::string& target_frame) {
//   geometry_msgs::msg::PointStamped point_out;

//   try {
//     // Wait for transform to be available (timeout: 1 second)
//     if (!tf_buffer_->canTransform(target_frame, point_in.header.frame_id,
//                                   tf2::TimePointZero,
//                                   tf2::durationFromSec(1.0))) {
//       RCLCPP_WARN(this->get_logger(), "Transform from %s to %s not
//       available",
//                   point_in.header.frame_id.c_str(), target_frame.c_str());
//       return point_out;  // Return empty point
//     }
...
//                  "Transformed point from %s to %s: (%.3f, %.3f, %.3f) -> "
//                  "(%.3f, %.3f, %.3f)",
//                  point_in.header.frame_id.c_str(), target_frame.c_str(),
...
//     return geometry_msgs::msg::PointStamped();  // Return empty point
//   }
...
//   // New parameter for dynamic board position
//   this->declare_parameter<bool>("use_dynamic_board_position", false);
...
//   use_dynamic_board_position_ =
//       this->get_parameter("use_dynamic_board_position").as_bool();

//   RCLCPP_INFO(this->get_logger(), "Parameters loaded successfully");
//   RCLCPP_INFO(this->get_logger(), "Dynamic board position: %s",
//               use_dynamic_board_position_ ? "ENABLED" : "DISABLED");
// }
...
//     const rclcpp_action::GoalUUID& uuid,
//     std::shared_ptr<const MoveAction::Goal> goal) {
//   // Check if we need to wait for board position
//   if (use_dynamic_board_position_ && !board_position_received_) {
//     RCLCPP_WARN(this->get_logger(),
//                 "Rejecting goal: waiting for board position from camera");
//     return rclcpp_action::GoalResponse::REJECT;
//   }

//   RCLCPP_INFO(this->get_logger(),
//               "Received request: phase=%d, type=%d, color=%d,
//               to_cell=(%d,%d)", goal->phase, goal->move.piece.type,
//               goal->move.piece.color, goal->move.to_cell.col,
...
//       RCLCPP_INFO(this->get_logger(), "PHASE_APPROACH: Moving to piece
//       center");
...
//       target_center_pose =
//           calculateWorldPose(target_col + offset_col, target_row +
//           offset_row);
...
//       // Then descend to target position
//       success = execute_pick_place(goal_handle, feedback,
//       target_center_pose); break;
//     }
...
//     std::shared_ptr<MoveAction::Feedback> feedback,
//     geometry_msgs::msg::PoseStamped target_pose) {
//   RCLCPP_INFO(this->get_logger(), "Moving to above target (APPROACH
//   height)");
...
//   // Calculate bottom-left corner position from board center
//   // board_center_x_ and board_center_y_ are now dynamically updated from
//   camera double grid_origin_x = board_center_x_ - board_width_ / 2.0; double
//   grid_origin_y = board_center_y_ - board_length_ / 2.0;
...
//   // Helper function to add joint constraint
//   auto addJointConstraint = [&](const std::string& joint_name, double
//   min_deg,
//                                 double max_deg, double weight) {
//     moveit_msgs::msg::JointConstraint constraint;
//     constraint.joint_name = joint_name;
...
//   RCLCPP_INFO(this->get_logger(),
//               "Applied %zu joint constraints to reduce planning search
//               space", constraints.joint_constraints.size());
...
//       sqrt(pow(target_pose.pose.position.x - current_pose.pose.position.x, 2)
//       +
//            pow(target_pose.pose.position.y - current_pose.pose.position.y,
//            2));
...
//   bool success = false;
//   if (fraction > cartesian_fraction_threshold_) {  // Use configurable
//   threshold
//     // Execute the Cartesian trajectory
//     moveit::planning_interface::MoveGroupInterface::Plan cartesian_plan;
//     cartesian_plan.trajectory_ = trajectory;
...
//     rt(move_group_interface_->getRobotModel(),
//                                          move_group_interface_->getName());
//     rt.setRobotTrajectoryMsg(*move_group_interface_->getCurrentState(),
//                              plan.trajectory_);
